...
const event=(groupId,text="AviCore ค้นหา เวลาพัก")=>({type:"message",replyToken:"test",source:{type:"group",groupId},message:{type:"text",text}});
...
it("searches addressed groups and ignores rooms",async()=>{
 const search=vi.fn().mockResolvedValue({query:{searchText:"rest"},mode:"hybrid",results:[]});
 await replyKnowledge({...event("friends"),source:{type:"group",groupId:"friends"}},{groupId:"pilot",search});
 await replyKnowledge({...event("pilot"),source:{type:"room",roomId:"pilot"}},{groupId:"pilot",search});
...
it("allows a friend direct chat to search",async()=>{
 ...
 expect(search).toHaveBeenCalledWith("เวลาพัก");expect(reply).toContain("ยังไม่พบ");
});
...
it("falls back to keyword when embedding fails, preserving tenant",async()=>{
 ...
 const result=await searchKnowledge(sb,{run:vi.fn().mockRejectedValue(new Error())},"tenant","fuel");
...
it("returns an explicit failure instead of a fabricated result",async()=>{
 const text=await replyKnowledge(event("pilot"),{groupId:"pilot",search:async()=>{throw Error();}});
